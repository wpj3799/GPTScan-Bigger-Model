...
def simple_cli():
    ...
    try:
        ...
        console.log(
            "[yellow]Since the compilation is failed, some static analysis tool may not be enabled, which may cause lower precision and recall.[/yellow]")
                            ...
                            if "static" in vul:
                                        ...
                                        if "multisteps" in vul["static"] and vul["static"]["multisteps"] == True:
                                                ...
                                                for filter_variable in vul["static"]["filter"]:
                                                    if filter_variable in answer:
                                                        for var in answer[filter_variable].copy():
                                                            ...
                                                            for target_feature in vul["static"]["filter"][
                                                                filter_variable]:
                                            ...
                                            if "format" in vul["static"] and vul["static"]["format"] == "json":
                                                answer, raw = analyze_pipeline.ask_for_static_json(
                                                    vul["static"]["prompt"], function1_text,
                                                    vul["static"]["output_keys"])
                                                if "validate_description" in vul["static"]:
                                                    for to_validate_key, to_validate_values in vul["static"][
                                                        "validate_description"].items():
                                                        validate_flag = True
                                                        if to_validate_key in raw and answer[to_validate_key] in raw[
                                                            to_validate_key]:
                                                            for v_line in to_validate_values:
                                                                v_line_flag = False
                                                                for v in v_line:
                                                                    if v.lower() in raw[to_validate_key][
                                                                        answer[to_validate_key]].lower():
                                                                        v_line_flag = True
                                                                        break
                                                                validate_flag = validate_flag and v_line_flag
                                                        if validate_flag == False:
                                                            raise Exception(
                                                                "The description of variable did not pass the `validate_description` validation")
                                                if "exclude_variable" in vul["static"]:
                                                    for to_exclude_key, to_exclude_values in vul["static"][
                                                        "exclude_variable"].items():
                                                        validate_flag = True
                                                        ...
                                                        if validate_flag == False:
                                                            raise Exception(
                                                                "The description of variable did not pass the `exclude_variable` validation")
                                                ...
                                                answer = analyze_pipeline.ask_for_static(vul["static"]["prompt"],
                                                                                         function1_text,
                                                                                         vul["static"]["output_keys"])
                                        ...
                                        if "multisteps" not in vul["static"] or vul["static"]["multisteps"] == False:
                                            for arg in vul["static"]["rule"]["args"]:
                                                if "CONSTANT" in arg:
                                                    ...
                                                    if "format" in vul["static"] and vul["static"][
                                                        "format"] == "json" or vul["static"]["format"] == "json_single":
                                                        args.append(answer[arg])
                                                    elif "format" in vul["static"] and vul["static"][
                                                        "format"] == "not_need":
                                ...
                                if function2 == "__ONLY_FUNCTION__":
                                    ...
                                    try:
                                        ...
                                        function2_detail = cg.get_function_detail(function2_file, function2_contract,
                                                                                  function2_func)
                                        function2_text = "\n".join(open(function2_file).read().splitlines()[int(
                                            function2_detail['loc']['start'].split(":")[0]) - 1:int(
                                        ...
                                        if "multisteps" in vul["static"] and vul["static"]["multisteps"] == True:
                                            ...
                                            if "format" in vul["static"] and vul["static"]["format"] == "json":
                                                answer, raw = analyze_pipeline.ask_for_static_json(
                                                    vul["static"]["prompt"], function2_text,
                                                    vul["static"]["output_keys"])
                                                if "validate_description" in vul["static"]:
                                                    for to_validate_key, to_validate_values in vul["static"][
                                                        "validate_description"].items():
                                                        ...
                                                        if validate_flag == False:
                                                            raise Exception(
                                                                "The description of variable did not pass the validation")
                                                if "exclude_variable" in vul["static"]:
                                                    for to_exclude_key, to_exclude_values in vul["static"][
                                                        "exclude_variable"].items():
                                                        validate_flag = True
                                        ...
                                        if "multisteps" not in vul["static"] or vul["static"]["multisteps"] == False:
                                            ...
                                            for arg in vul["static"]["rule"]["args"]:
                                                if "CONSTANT" in arg:
                                                    ...
                                                    args.append(answer[arg])
                                        res_2 = static_check.run_static_check(checker, args, function2_func,
    ...
    meta_data["estimated_cost"] = (meta_data["token_sent"] * global_config.SEND_PRICE) + (
